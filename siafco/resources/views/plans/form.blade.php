<x-layouts.app title="{{ $plan->exists ? 'Editar plan' : 'Nuevo plan' }}">
    <form method="post" action="{{ $plan->exists ? route('plans.update', $plan) : route('plans.store') }}" class="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 md:grid-cols-2">
        ...
        <div>
            ...
            <input class="form-input" name="name" value="{{ old('name', $plan->name) }}" data-uppercase required>
        </div>
        ...
        <div>
            ...
            <input class="form-input" name="description" value="{{ old('description', $plan->description) }}" data-uppercase>
        </div>
        ...
        <div class="md:col-span-2"><label class="form-label">Instrucciones de pago</label><textarea class="form-input" name="payment_instructions" rows="3" data-uppercase>{{ old('payment_instructions',$plan->payment_instructions) }}</textarea></div>
