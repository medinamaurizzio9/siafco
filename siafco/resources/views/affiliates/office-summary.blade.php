    ...
    <div class="grid gap-5 xl:grid-cols-[1fr_360px]">
        <section class="rounded-lg border {{ \App\Support\PaymentStatus::isConfirmed($payment->status) ? 'border-emerald-200' : 'border-amber-300' }} bg-white p-5 shadow-sm">
            <div class="mb-5 rounded-lg border {{ \App\Support\PaymentStatus::isConfirmed($payment->status) ? 'border-emerald-200 bg-emerald-50 text-emerald-900' : 'border-amber-300 bg-amber-50 text-amber-900' }} p-4">
                ...
                <p class="mt-1 text-sm">{{ \App\Support\PaymentStatus::isConfirmed($payment->status) ? 'La afiliacion presencial quedo activa segun las reglas de confirmacion de pagos.' : 'El pago QR fue registrado y debe ser verificado por Gerencia antes de activar la afiliación.' }}</p>
            </div>
            ...
            <dl class="grid gap-4 md:grid-cols-2">
                ...
                <div><dt class="text-xs font-black uppercase text-slate-500">Metodo</dt><dd>{{ $payment->source === 'office_qr' ? 'QR / Transferencia' : 'Efectivo / Oficina' }}</dd></div>
                @if($payment->source === 'office_qr')<div><dt class="text-xs font-black uppercase text-slate-500">N.º de transferencia / operación</dt><dd>{{ $payment->reference_number }}</dd></div>@endif
